 ...
 * 테스트로 교차 검증).
 */
final class InternalAuthTestTokens {

  private InternalAuthTestTokens() {}

  static String signedWithUser(String secret, String userId, Instant iat, Instant exp) {
    return sign(secret, JWSAlgorithm.HS256, claims(userId, null, iat, exp));
  }

  static String signedWithSession(String secret, String sessionToken, Instant iat, Instant exp) {
    return sign(secret, JWSAlgorithm.HS256, claims(null, sessionToken, iat, exp));
  }

  /** 다른 알고리즘(HS384)으로 서명 — HS256 고정이 이를 거부하는지(알고리즘 핀) 확인용. */
  static String signedWithAlg(
  ...
  static String unsignedNone(String userId, Instant iat, Instant exp) {
    return new PlainJWT(claims(userId, null, iat, exp)).serialize();
  }

  static String signedWithClaims(String secret, JWTClaimsSet claims) {
    return sign(secret, JWSAlgorithm.HS256, claims);
  }

  static JWTClaimsSet claims(String userId, String sessionToken, Instant iat, Instant exp) {
