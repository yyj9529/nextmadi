 ...
 * scaffold context, mirroring {@link com.phraselog.ai.logging.NoOpAiRequestLogStore}). The analysis
 * endpoints cannot function without a database, so every method fails fast; the no-DB context only
 * exercises context loading and actuator, never {@code /analysis}.
 */
final class UnavailableAnalysisRepository implements AnalysisRepository {
