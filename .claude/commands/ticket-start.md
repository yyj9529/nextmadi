...
위 이슈를 처리한다. **두 문서를 함께 따른다**: `docs/ai-native-dev-guide.md`(티켓별 배정 — 누가/뭘 보고)와 `docs/harness.md` 7절(공통 진행 순서 — 어떤 순서로). 카드의 배정을 harness 워크플로 흐름(`/ticket-start → /ce-plan → 구현 → /spec-check → /ce-code-review → /ui-verify → ...`)에 끼워 넣어 다음 단계를 제시하고, 매 단계 owner 확인을 받는다. 다음 순서로 진행해라.
