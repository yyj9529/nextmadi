...
Judge model: Anthropic Claude, and it must be a different model from the one generating the S07 output. Anthropic's evaluation guidance states that rule without giving a reason for it, as a comment repeated across its grader examples: "Generally best practice to use a different model to evaluate than the model used to generate the evaluated output" (source https://platform.claude.com/docs/en/test-and-evaluate/develop-tests , verified 2026-09-09). That page does not mention self-preference bias or model families.

The reason comes from the research literature instead. Judges inflate scores for outputs from their own family, not only for their own outputs: measured at 8.9 percent for same-family pairs against 23.6 percent for the same model (Preference Leakage, https://arxiv.org/abs/2502.01534 ), attributed to familiarity rather than authorship since judges favour low-perplexity text regardless of who wrote it ( https://arxiv.org/abs/2410.21819 ), and reproduced under rubric-based absolute scoring rather than only pairwise comparison ( https://arxiv.org/pdf/2604.06996 ). All three verified 2026-09-09.

The current pairing satisfies the different-model rule but is still same-family, so the 8.9 percent figure applies to it. A constant judge bias largely cancels in the one claim this eval makes, a paired comparison against the previous baseline on the same cases, but it does not cancel in the absolute pass thresholds below. Two gaps are open and neither is measured: no cross-family judge has been run against the same cases, and no published measurement of the current judge model acting as a judge was found. Cultural and tone dimensions are the exposed ones, since judges agree with humans less on culturally specific nuance and carry an English fluency prior ( https://arxiv.org/html/2607.02235v1 , verified 2026-09-09).

From 2026-06 to 2026-09 the generator and judge were both Sonnet; that is recorded as a known gap, and baselines from that period are not comparable with later ones. The concrete model ids live in `eval/s07-analysis/run_eval.py` and `AI_PIPELINE.md`. Quality of judging matters more than speed, so the judge is never the cheapest tier.
