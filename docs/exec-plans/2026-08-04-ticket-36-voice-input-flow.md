...
- **리뷰 should-fix 5건 처리 (2026-08-27)**:
  ...
  - **남은 것 — `use-voice-recorder` 테스트**: 저장소에 React 테스트 인프라가 없다
    (테스트 라이브러리·DOM 환경·bunfig 모두 없고, 기존 테스트는 순수 모듈과 라우트뿐).
    훅 렌더링 테스트는 새 의존성 결정이라 별건으로 남긴다. 교착의 뿌리는 순수 리듀서에
    있었으므로 그쪽에 회귀 테스트를 넣어 두었다.
