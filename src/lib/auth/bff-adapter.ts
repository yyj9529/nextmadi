 ...
 * 약하다.
 *
 * 첫째, `package.json`이 캐럿 범위의 프리릴리스를 가리키고 있었다. 보안상 의미 있는 수명의
 * 유일한 집행자가 우리가 고르지 않은 버전에 있었다는 뜻이다. 지금은 정확 버전으로 고정했지만,
 * 집행자를 하나로 두는 구조 자체가 다음 업그레이드마다 같은 질문을 되살린다.
 *
 * 둘째, 그 비교는 `expires`가 파싱되는 값일 때만 동작한다. `new Date(...)`가 `Invalid Date`가
 * 되면 `NaN < Date.now()`는 **false** — 만료 판정이 조용히 뒤집혀 만료된 링크가 영구히
 * 유효해진다. 그래서 `> Date.now()` 앞에 유한성부터 확인한다. 이 방향의 실패는 열리는 쪽이
 * 아니라 닫히는 쪽이어야 한다.
 *
 * 유한성 검사는 파싱 가능성만 본다. 계약이 요구하는 ISO 문자열인지까지 보지는 않는다 —
 * epoch 밀리초 숫자나 RFC 날짜 문자열은 파싱되고, 파싱된 순간이 옳으면 판정도 옳다.
 * 계약 자체를 고정하는 것은 #19 리뷰가 남긴 별도 항목(wire-format pinning)이다.
 *
